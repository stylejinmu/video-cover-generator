...
根据单张原图自动生成适配各大主流视频平台的封面图。原图内容完整保留，因比例差异产生的留白区域由 OpenAI `gpt-image-2` 扩图技术（outpainting）自然补全。

## 核心特性

- **多平台适配**：支持微信视频号、抖音、快手、B站、小红书等 5 大平台，共 9 种主流规格（横版/竖版）。
- **AI 扩图补全**：采用 OpenAI `gpt-image-2` 对留白区域进行内容延伸，主体完整居中保留，告别黑边与生硬裁切。
...
3. **AI 补全**：通过 `images.edit` API 将透明区域交由 `gpt-image-2` 根据原图风格自然延伸填充。
